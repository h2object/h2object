...
func (m *Message) RenderTemplate(mimetype string, templatePath string, args map[string]interface{}) error {
	switch mimetype {
	case "html":
		doc, err := m.renderViewTemplate(templatePath, args)
		if err !=nil {
			return err
		}
		m.HtmlBody = doc
...
func (m *Message) renderViewTemplate(templateFilePath string, args map[string]interface{}) (*bytes.Buffer, error) {
	tmplBytes, err := ioutil.ReadFile(templateFilePath)
	if err != nil {
		return nil, err
	}

	var doc bytes.Buffer
	t := template.Must(template.New("content").Parse(string(tmplBytes)))
